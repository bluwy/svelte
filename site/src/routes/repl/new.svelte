...
<script>
	...
	import helloWorldHash from './_utils/helloWorldHash';
	import InputOutputToggle from '../../components/Repl/InputOutputToggle.svelte';
	import AppControls from './_components/AppControls.svelte';
	...
	function update_query_string(version) {
		...
		history.replaceState({}, 'x', url.href);
	}
	...
	onMount(() => {
		...
		// Initially decode hash, subsequent code changes are handled by AppControls
		decodeCodeHash();
	});

	function decodeCodeHash() {
		const hash = window.location.hash || (window.location.hash = helloWorldHash);

		const stringData = LZString.decompressFromEncodedURIComponent(hash.slice(1));

		const data = JSON.parse(stringData);
		
		// Name doesn't update in AppControls if no tick, weird
		tick().then(() => name = data.name);
